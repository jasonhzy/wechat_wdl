...
<div class="alert alert-block alert-new">
	...
	<table>
		<tbody>	
			<tr>
				<th>活动状态</th>
				<td>
				    <label for="aaa" class="radio inline"><input id="aaa" type="radio" name="doings" value="1" <?php  if($reply['status'] == 1) { ?>checked="checked"<?php  } ?>> 开始活动</label>
					<label for="bbb" class="radio inline"><input id="bbb" type="radio" name="doings" value="0" <?php  if($reply['status'] == 0) { ?>checked="checked"<?php  } ?>> 关闭活动（前台显示为暂停）</label>
				</td>
			</tr>
			<tr>
				<th>活动标题</th>
				<td><input type="text" name="title" value="<?php  echo $reply['title'];?>" class="span7"> </td>
			</tr>
            <tr>
				<th>活动时间</th>
			...
			<tr>
				...
				<td>
						...
						<input type="text" value="<?php  echo $reply['picture'];?>" name="picture" id="upload-image-url-picture" class="span3" autocomplete="off">
						<input type="hidden" value="<?php  echo $picture;?>" name="picture_old" id="upload-image-url-picture-old">
						<input name="shouquan" type="hidden" value="<?php  echo $shouquan;?>" ><input name="we7_ValidCode_server" type="hidden" id="we7_ValidCode_server" ><button class="btn" type="button" id="upload-image-picture">选择图片</button>
						</div>
						<div id="upload-image-preview-picture" style="margin-top:10px;"><img src="<?php  echo $picture;?>" width="100%" /></div><div class="help-block">大图片建议尺寸：900像素 * 500像素</div>
				</td>
			</tr>
			<tr>
				<th>活动简介</th>
			...
			<tr>
				<th>活动详情</th>
				<td>
					<textarea style="height:200px; width:535px;" class="span7 richtext-clone" name="content" cols="70" id="reply-add-text"><?php  echo $reply['content'];?></textarea>					
			...
			<tr>
				<th>分享标题</th>
				<td>
					<input type="text" name="sharetitle" value="<?php  echo $reply['sharetitle'];?>" class="span7"> 
			...
			<tr>
				<th>分享内容</th>
				<td>
					<textarea style="height:50px;" name="sharecontent" class="span7" cols="60"><?php  echo $reply['sharecontent'];?></textarea>
					<div class="help-block">此设置用于领取礼盒后让别人帮拆礼盒时的分享语，且分享到朋友圈时只显示标题</div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th>参与是否关注</th>
				<td><label for="subscribe_1" class="radio inline"><input id="subscribe_1" type="radio" name="subscribe" value="1" <?php  if($reply['subscribe'] == 1) { ?>checked="checked"<?php  } ?>> 关注参与</label>
					<label for="subscribe_0" class="radio inline"><input id="subscribe_0" type="radio" name="subscribe" value="0"  <?php  if($reply['subscribe'] == 0) { ?>checked="checked"<?php  } ?>> 任意参与</label>
				    <div class="help-block">是否强制用户关注公众号才能参与活动</div></td>
			</tr>
			<tr>
				<th>引导关注网址</th>
				<td>
				<input type="text" name="shareurl" value="<?php  echo $reply['shareurl'];?>" class="span7"> 
				<div class="help-block">你可以在公众号平台素材库里新建图文，把网址填在上面。未关注用户即可通过此网址引导一键关注，也只有官方的公众号平台素材库才能做这个一键关注功能！</div>
				</td>
			</tr>			
			<tr>
				<th>奖品资格</th>
				<td><label for="opensubscribe_4" class="radio inline"><input id="opensubscribe_4" type="radio" name="opensubscribe" value="4" <?php  if($reply['opensubscribe'] == 4) { ?>checked="checked"<?php  } ?>> 参与活动</label>
					<label for="opensubscribe_3" class="radio inline"><input id="opensubscribe_3" type="radio" name="opensubscribe" value="3"  <?php  if($reply['opensubscribe'] == 3) { ?>checked="checked"<?php  } ?>> 好友助威</label>
					<label for="opensubscribe_2" class="radio inline"><input id="opensubscribe_2" type="radio" name="opensubscribe" value="2" <?php  if($reply['opensubscribe'] == 2) { ?>checked="checked"<?php  } ?>> 参与访问</label>
					<label for="opensubscribe_1" class="radio inline"><input id="opensubscribe_1" type="radio" name="opensubscribe" value="1"  <?php  if($reply['opensubscribe'] == 1) { ?>checked="checked"<?php  } ?>> 参与互访</label>
					<label for="opensubscribe_0" class="radio inline"><input id="opensubscribe_0" type="radio" name="opensubscribe" value="0"  <?php  if($reply['opensubscribe'] == 0) { ?>checked="checked"<?php  } ?>> 访问活动</label>
			...
			<tr>
				<th>是否互访</th>
				<td><label for="isvisits_1" class="radio inline"><input id="isvisits_1" type="radio" name="isvisits" value="1" <?php  if($reply['isvisits'] == 1) { ?>checked="checked"<?php  } ?>> 开启互访</label>
					<label for="isvisits_0" class="radio inline"><input id="isvisits_0" type="radio" name="isvisits" value="0"  <?php  if($reply['isvisits'] == 0) { ?>checked="checked"<?php  } ?>> 禁止互访</label>
			...
			<tr>
				...
				<td><div class="input-prepend input-append">
                    ...
                    <input type="text" class="span1" name="biaobiaonum" id="biaobiaonum" value="<?php  echo $reply['biaobiaonum'];?>" />
			...
			<tr>
				...
				<td><div class="input-prepend input-append">
                    ...
                    <input type="text" class="span1" name="xuninum" id="xuninum" value="<?php  echo $reply['xuninum'];?>" />
			...
			<tr>
				...
				<td><div class="input-prepend input-append">
                    ...
                    <input type="text" class="span1" name="xuninumtime" id="xuninumtime" value="<?php  echo $reply['xuninumtime'];?>" />
                    <span class="add-on">秒　系统自动增加</span>
					<input type="text" class="span1" name="xuninuminitial" id="xuninuminitial" value="<?php  echo $reply['xuninuminitial'];?>" />
					<span class="add-on">至</span>
					<input type="text" class="span1" name="xuninumending" id="xuninumending" value="<?php  echo $reply['xuninumending'];?>" />
			...
			<tr>
				...
				<td><div class="input-prepend input-append">
                    <span class="add-on">每加载一次显示</span>
                    <input type="text" class="span1" name="share_shownum" id="share_shownum" value="<?php  echo $reply['share_shownum'];?>" />
            ...
            <tr>
				<th>参与提示语</th>
				<td>
					<input type="text" name="userinfo" value="<?php  echo $reply['userinfo'];?>" class="span7"> 
					<div class="help-block">需要用户输入信息时的提示语</div>
				</td>
			</tr>
			<tr>
				<th>参数项同步</th>
				<td><label for="isfans_1" class="radio inline"><input id="isfans_1" type="radio" name="isfans" value="1" <?php  if($reply['isfans'] == 1) { ?>checked="checked"<?php  } ?>> 参数项同步更新至官方FANS表中</label>
					<label for="isfans_0" class="radio inline"><input id="isfans_0" type="radio" name="isfans" value="0"  <?php  if($reply['isfans'] == 0) { ?>checked="checked"<?php  } ?>> 参数项只保留本模块下</label></td>
			</tr>
			<tr>
				<th>版权显示</th>
				<td><label for="iscopyright_1" class="radio inline"><input id="iscopyright_1" type="radio" name="iscopyright" value="0" <?php  if($reply['iscopyright'] == 0) { ?>checked="checked"<?php  } ?>> 公众号版权</label>
					<label for="iscopyright_0" class="radio inline"><input id="iscopyright_0" type="radio" name="iscopyright" value="1"  <?php  if($reply['iscopyright'] == 1) { ?>checked="checked"<?php  } ?>> 自定义版权</label></td>
			</tr>
			<tr>
				<th>版权设置</th>
				<td>
					<input type="text" name="copyright" value="<?php  echo $reply['copyright'];?>" class="span2"> <input type="text" name="copyrighturl" value="<?php  echo $reply['copyrighturl'];?>" class="span5"> 
	...
	<div id="append-list" class="list">
		<?php  if(!empty($award)) { ?>
			...
			<div class="item" id="ccc-item-<?php  echo $item['id'];?>">
...
<script type="text/javascript">
	...
	$("#upload-image-awardpic<?php  echo $item['id'];?>").click(function() {
		editor.loadPlugin("image", function() {
			editor.plugin.imageDialog({
				tabIndex : 1,
				imageUrl : $("#upload-image-url-awardpic<?php  echo $item['id'];?>").val(),
				clickFn : function(url) {
					...
					$("#upload-image-url-awardpic<?php  echo $item['id'];?>-old").val($("#upload-image-url-awardpic<?php  echo $item['id'];?>").val());
					$("#upload-image-url-awardpic<?php  echo $item['id'];?>").val(val);
					$("#upload-image-preview-awardpic<?php  echo $item['id'];?>").html('<img src="'+url+'" width="180" />');
				}
			});
		});
	});
